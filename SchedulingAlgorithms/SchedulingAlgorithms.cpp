...
public:
  ...
  int end;
  int timeRan = 0;
  SolvedProc(const PCB &pcb, int start, int end)
      : PCB(pcb), start(start), end(end) {}
};
...
// for some reason priority queue is accepting the the reverse of comparator
// required by std::sort if the same comparator is used in std::sort it'll sort
// descending using it in queue however gives elements in ascending order
...
auto priorityComparator = [](const PCB &pcb1, const PCB &pcb2) {
  if (pcb1.priority > pcb2.priority)
    return true;
  if (pcb2.priority > pcb1.priority)
    return false;
...
auto shortestJobComparator = [](const PCB &pcb1, const PCB &pcb2) {
  if (pcb1.burst > pcb2.burst)
    return true;
  if (pcb2.burst > pcb1.burst)
    return false;
...
Solution fcfs(std::vector<PCB> &pcbs) {
  ...
  for (int i = 0; i < pcbs.size();) {
    ...
    if (pcb.arrival > timeLapsed) {
      ...
      solution.ganttChart.emplace_back(pcb.pid, timeLapsed,
                                       timeLapsed + pcb.burst);
      solution.solutionTable.emplace_back(pcb, timeLapsed,
                                          timeLapsed + pcb.burst);
      timeLapsed += pcb.burst;
      i++;
    }
...
Solution hpf(std::vector<PCB> &pcbs,
             std::function<bool(const PCB &, const PCB &)> comparator =
                 priorityComparator) {
  std::sort(pcbs.begin(), pcbs.end(), arrivalTimeComparator);

  Solution solution;

  std::priority_queue<SolvedProc, std::vector<SolvedProc>, decltype(comparator)>
  ...
  while (i < pcbs.size() || !readyQueue.empty() || timeLapsed == 0) {
    ...
    if (!readyQueue.empty()) {
      ...
      solution.ganttChart.emplace_back(proc.pid, timeLapsed,
...
Solution preemptiveHpf(std::vector<PCB> pcbs,
                       std::function<bool(const PCB &, const PCB &)>
...
void ganttPrinter(const std::vector<GanttDescriptor> &chart) {
  for (auto &desc : chart) {
    if (desc.pid == -1) {
      ...
      std::cout << "process " << desc.pid << " from " << desc.start << " to "
                << desc.end << "\n";
    }
  }
}
...
void printTable(const std::vector<SolvedProc> &procs) {
  std::cout << std::setw(5) << "PID" << std::setw(10) << "ARRIVAL"
            << std::setw(10) << "BURST" << std::setw(10) << "PRORITY"
            << std::setw(10) << "START" << std::setw(10) << "END"
            << std::setw(10) << "TAT" << std::setw(10) << "WT"
            << "\n";

  for (const SolvedProc &proc : procs) {
    std::cout << std::setw(5) << proc.pid << std::setw(10) << proc.arrival
              << std::setw(10) << proc.burst << std::setw(10) << proc.priority
              << std::setw(10) << proc.start << std::setw(10) << proc.end
              << std::setw(10) << (proc.end - proc.arrival) << std::setw(10)
              << (proc.end - proc.arrival - proc.burst) << "\n";
  }
}

int main() {
  std::vector<PCB> pcbs{{1, 3, 10, 5}, {2, 2, 1, 1}, {3, 4, 2, 3},
                        {4, 10, 6, 4}, {5, 6, 5, 2}, {6, 28, 1, 0}};

  ////{ {1, 0, 4, 2}, {2,1,2,4},{3, 2, 3, 6 }, {4,3, 5, 10},{5,4, 1, 8},{6, 5,
  ///4, 12},{7, 6, 6, 9} };
  ////{ {1, 0, 3,2}, {2, 2, 5, 6}, {3, 1, 4, 3},{4, 4, 2, 5 }, {5, 6, 9, 7}, {6,
  ///4, 5, 4 }, {7,10, 7, 10} };
  ...
  // std::cout << proc1.pid << " " << proc1.arrival << " " << proc1.burst << " "
  // << proc1.priority << " " << proc1.start << " " << proc1.end;
}
